...
Scripts to relay media files (video/media) from Garmin inreach messenger plus to email or matrix chats. The way it currently works is that it will relay media from Garmin messenger installed on e.g. waydroid &amp; raspberry pi (or another linux system), to an email or matrix chat of your choice. Other solutions are also possible here for different ways of running an android system, e.g. docker redroid is what i am currently using, which seems to be working well for this purpose. You need to install and run garmin messenger on android system/emulator of your choice, and log in with a Garmin messenger account (your own, or create a separate relay account with a valid phone number.)
